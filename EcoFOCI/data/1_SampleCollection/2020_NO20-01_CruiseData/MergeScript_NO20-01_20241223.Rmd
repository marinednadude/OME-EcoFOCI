...
sample_Data %>% 
  ...
  mutate(.,  Date_UTC = as.POSIXct(Combined_date, format="%m/%d/%Y %H:%M", tz="UTC"),
         ...
         Date_local_check = if_else(is.na(Date_local_check), as.POSIXct(Combined_date_local, format="%m/%d/%y", tz="US/Alaska"), Date_local_check )) %>% 
    mutate(., Date_UTC= format(Date_local_check,tz="UTC")) %>% 
      mutate(., Date_local= format(as.POSIXct(Date_UTC, format="%Y-%m-%d %H:%M", tz="UTC"),tz="US/Alaska") ) %>% 
  ...
  mutate(., , Lon_dec = Lon_dec*-1) %>% 
    mutate(., Cast= as.character(Cast_No.),
         Bottle=as.character(Rosette_position)) -> QAQCMRE_CruiseSampleMetadata_NO20_01
...
colnames(QAQCMRE_CruiseSampleMetadata_NO20_01)
setdiff(skq1_colnames,colnames(QAQCMRE_CruiseSampleMetadata_NO20_01) )
setdiff(colnames(QAQCMRE_CruiseSampleMetadata_NO20_01),skq1_colnames )

QAQCMRE_CruiseSampleMetadata_NO20_01 <- QAQCMRE_CruiseSampleMetadata_NO20_01[, skq1_colnames]

readr::write_excel_csv(QAQCMRE_CruiseSampleMetadata_NO20_01, file="QAQCMRE_CruiseSampleMetadata_NO20-01.csv")
```


```{r}

min_lat <- min(QAQCMRE_CruiseSampleMetadata_NO20_01$Lat_dec, na.rm = T)
max_lat <- max(QAQCMRE_CruiseSampleMetadata_NO20_01$Lat_dec, na.rm = T)

min_lon <- min(QAQCMRE_CruiseSampleMetadata_NO20_01$Lon_dec, na.rm = T)
max_lon <- max(QAQCMRE_CruiseSampleMetadata_NO20_01$Lon_dec, na.rm = T)

world <- ne_countries(scale = "medium", returnclass = "sf")
QAQCMRE_CruiseSampleMetadata_NO20_01$Date_UTC %>% as.Date() -> dates


ggplot(data = world) +
    geom_sf() +
    geom_point(data = QAQCMRE_CruiseSampleMetadata_NO20_01, aes(x = Lon_dec, y = Lat_dec, colour=as.Date(Date_UTC)), size=2) +
    coord_sf(xlim = c(min_lon-1, max_lon+1), ylim = c(min_lat-1, max_lat+1), expand = FALSE) +theme_bw() +xlab("Longitude") +ylab("Latitude") +
  geom_text(data = QAQCMRE_CruiseSampleMetadata_NO20_01, aes(x = Lon_dec, y = Lat_dec, label=Station),size = 2, colour="black") + scale_color_cmocean(labels=dates, breaks=dates, name ="tempo", start = 0.1, end=0.5) +labs(colour="Date")


```

```{r}
QAQCMRE_CruiseSampleMetadata_NO20_01 %>% 
...
```{r}
bottle_Data <- read_excel(here("CTD_raw_BottleFile_NO20-01.xlsx"))
...
```{r}
QAQCMRE_CruiseSampleMetadata_NO20_01 %>% 
  left_join(bottle_Data, by=c("Cast_No."="btl_Consecutive Cast Number","Rosette_position"="btl_Bottle")) -> NO2020_sample_data
...
NO2020_sample_data %>% 
  ...
  dplyr::select(Sample_Name, 
         ...
         #!!!syms(cols_2_keep),
          starts_with("btl")) -> NO2020_sample_data
