...
FinalOME_Merge_DY20_12 %>% 
  ...
  dplyr::select(Sample_Name, Biological_Replicate, Technical_Replicate, Negative_control, Cruise_ID_short, Cruise_ID_long, Cast_No.,Rosette_position, Station, Sample_volume_ml, Field_collection_method, Preservation, pH, pH_method, Silicate_umol, Silicate_umol.per.kg, Tot_Inorganic_N.umol, wind_direction, verbatimLongitude, verbatimLatitude, verbatimCoordinateSystem, verbatimEventDate, verbatimEventTime,
                Time=ctd_time..UTC., 
...
```{r}
# adjust sample names in extraction sheet to remove hypehns with periods (in -12S samples)
alaska_arctic22_23_extraction_df %>% 
  mutate(., 'FINAL Sample NAME' = gsub("-", ".", 'FINAL Sample NAME')) %>%
  dplyr::select('FINAL Sample NAME', 
                qubit_conc_ng.uL = 'Qubit Conc.  (ng/μL)') -> alaska_arctic22_23_extraction_df

# # adjust skq21 sample name field negatives to to remove the E. in the front
...
revamp_sample_metadata <- merge(
  x = revamp_sample_metadata,
  y = extractions_df[, c("FINAL Sample NAME", "qubit_conc_ng.uL")],
  by.x = "Sample_Name",
  by.y = "FINAL Sample NAME",
  all.x = TRUE
)
```
...
# Join and filter keeping PCR from revamp_format_run1-4 and NC from sample_metadata
```{r}
...
run1_4_revamp_formatted_metadata <- full_join(
  revamp_sample_metadata, revamp_format_run1_4, by= c("Sample_Name" = "join_key")) %>%
            filter(
  ...
  mutate(., PCR_technical_replicate = case_when(grepl("\\.PCR", Sample_Name.y) ~ Sample_Name, TRUE ~ PCR_technical_replicate)) %>%
  mutate(., Sample_Name = case_when(grepl("\\PCR", Sample_Name.y) ~ Sample_Name.y, TRUE ~ Sample_Name)) %>%
